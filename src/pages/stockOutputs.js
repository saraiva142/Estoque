...
const StockOutputs = () => {
  ...
  const [product_day, setProduct_day] = useState("");
  const [product_day_ret, setProduct_day_ret] = useState("");
  ...
  const handleNewOutput = () => {
    ...
    if (listStockOutputs && listStockOutputs.length) {
      localStorage.setItem(
        "db_stock_outputs",
        JSON.stringify([...listStockOutputs, { id, amount, product_id, product_day, product_day_ret }])
      );

      setStockOutputs([...listStockOutputs, { id, amount, product_id, product_day, product_day_ret }]);
    } else {
      localStorage.setItem(
        "db_stock_outputs",
        JSON.stringify([{ id, amount, product_id, product_day, product_day_ret }])
      );

      setStockOutputs([{ id, amount, product_id, product_day, product_day_ret }]);
    }
        ...
        <Box w="100%">
          <SimpleGrid minChildWidth={240} h="fit-content" spacing="6">
            ...
            <Input
              ...
              type="text"
              value={product_day_ret}
              onChange={(e) => setProduct_day_ret(e.target.value)}
            />
            <Input
              placeholder="Data de Entrega"
              type="text"
              value={product_day}
              onChange={(e) => setProduct_day(e.target.value)}
            />
              ...
              <Tbody>
                {listStockOutputs.map((item, i) => (
                  <Tr key={i}>
                    ...
                    <Td color="gray.500">{item.product_day_ret}</Td>
                    <Td color="gray.500">{item.product_day}</Td>
                    <Td textAlign="end">
                      <Button
                        p="2"
                        h="auto"
